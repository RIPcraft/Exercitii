...
TablaÎnmulțirii.prototype.generează = function () {
    ...
    function generate() { var n = Math.floor(Math.random() * 199) - 99; return n > 0 ? { p: true, s: '+', v: n, a: n } : { p: false, s: '-', v: n, a: Math.abs(n) } };
    var a = generate(), b = generate(), c = a.v + b.v, cp = a >= 0; c = { p: cp, s: cp ? '+' : '-', v: c, a: Math.abs(c) };
    // afișarea valorilor
    this.termeni[0].innerHTML = a.v;
    this.termeni[1].innerHTML = '&nbsp;' + b.s + '&nbsp;';
    this.termeni[2].innerHTML = b.a;
    this.termeni[3].innerHTML = '&nbsp;=&nbsp;';
    this.termeni[4].innerHTML = c.v;
    this.obiectivTastatură(this.termeni[Math.floor(Math.random() * 3) * 2]);
    var _this = this;
